...
export default function BottomNav({ onOpenMore }: BottomNavProps) {
 ...
 return (
 <nav 
 className="md:hidden fixed bottom-0 inset-x-0 bg-white/95 backdrop-blur-md border-t border-slate-200 z-30 pb-safe shadow-[0_-4px_16px_rgba(0,0,0,0.08)] print:hidden"
 ...
 <span className="absolute -top-1 -left-2 bg-rose-600 text-white text-[9px] font-black rounded-full h-4 min-w-[16px] px-1 flex items-center justify-center shadow-xs animate-pulse">
 {item.badge}
 </span>
 )}
 </div>
 <span className={cn('truncate max-w-[50px] leading-tight', isActive ? 'font-extrabold text-[#800020] ' : 'font-medium')}>
 {item.name}
 </span>
 {isActive && (
 <span className="absolute top-1 w-6 h-1 bg-[#800020] rounded-full" />
 )}
 </>
 )}
 </NavLink>
 ...
 type="button"
 onClick={onOpenMore}
 className="flex flex-col items-center justify-center h-full py-1 text-[11px] font-medium text-slate-500 hover:text-slate-700 transition-all rounded-xl active:scale-95 bg-transparent border-none cursor-pointer"
 >
