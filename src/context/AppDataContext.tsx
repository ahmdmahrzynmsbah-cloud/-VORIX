...
export function AppDataProvider({ children }: { children: ReactNode }) {
  ...
  const [businessProfile, setBusinessProfile] = useState<BusinessProfile>(() => {
    const stored = scanStorageForItems<BusinessProfile>('doctor_tools_profile', profileLegacyKeys, isProfile, {
      ...
      phone: '',
      address: 'القاهرة، مصر',
      description: 'نظام VORIX المتكامل لإدارة قطع الغيار والمخزون والفواتير',
      logo: '/logo.png'
    });
    // Migrate old Doctor Tools default name if present
    if (!stored.name || stored.name === 'Doctor Tools') {
      stored.name = 'VORIX';
    }
    if (!stored.description || stored.description.includes('العيادات والمستلزمات الطبية')) {
      stored.description = 'نظام VORIX المتكامل لإدارة قطع الغيار والمخزون والفواتير';
    }
  ...
  const updateBusinessProfile = useCallback(async (profile: BusinessProfile) => {
    const dataToSave = {
      name: profile.name || 'VORIX',
      phone: profile.phone || '',
      address: profile.address || 'القاهرة، مصر',
      description: profile.description || 'نظام VORIX المتكامل لإدارة قطع الغيار والمخزون والفواتير',
